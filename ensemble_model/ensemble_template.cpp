...
namespace ensemble
{
    ...
    void ensemble_simulate(
        ...
        double strain[], double dt[], double out_stress[], int n)
    {
    ...
    template<class matlaw, ResidualFunction resfunc, HysteresisFunction hystfunc, PenaltyFunction penfunc>
    double calc_ensemble_objective(
        double pars[], double visco[], double Tf, double Cmax[],
        double strain[], double stress[], double dt[],
        double deltaCG[], double hysteresis,
        int n, int skip
    ) {
    ...
    double residual_body_below(double strain[], double stress[], int n, int skip, double sims[], double baseline
    ){
        double res = 0.0;
        for (int k = 0; k < n; k++) {
    ...
    double residual_body_quadratic(double strain[], double stress[], int n, int skip, double sims[], double baseline
    ){
        double res = 0.0;
        for (int k = 0; k < n; k++) {
    ...
    double penalty_function_null(double pars[], double visco[]) {return 1.0;};
    ...
    double penalty_body_ensemble3(double pars[],  double visco[]) {
        ...
        return 1.0 + 1.0*k_e*k_e + 0.1*b_s*b_s
            + 10.0*v_c*v_c + 10.0*v_s*v_s;
    }
    ...
    double thoracic_smc_ensemble_residual(
        double pars[], double visco[], double Tf, double Cmax[],
        double args[], double stress[], double dt[],
        double deltaCG[], double hysteresis, int n, int skip
    ) {
        return calc_ensemble_objective<
            ThoracicSMCEnsembleVE, residual_body_below, hysteresis_body_null, penalty_body_ensemble2
            >(
                pars, visco, Tf, Cmax,
                args, stress, dt, deltaCG, hysteresis, n, skip
            );
    }
