...
#include <cmath>
#include "thoracic_iso_model.hpp"
...
/*----------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------*/
    ...
    void ThoracicIsoBase::get_scaled_pars(double pars[])
    {
        pars[0] = m_elastin.mu;
        pars[1] = m_muscle.get_scaled_modulus();
        // pars[2] = m_muscle.m_b;
        pars[2] = m_collagen.get_scaled_modulus();
        pars[3] = m_collagen.k2;
    }


    void ThoracicIsoBase::stress(const kinematics::kinematics<4> &kin, const double dt, double stress[])
    {
        double p = 0.0;
    ...
    void ThoracicIsoVEBase::stress(const kinematics::kinematics<4> &kin, const double dt, double stress[])
    {
        double p = 0.0;
        double el[4], smc[4], col[4];
        p = p + m_elastin.stress(kin, el);
