...
void leastsquareM_he(
    ...
    double weights[], int index[], int select[], int n, int dim, int nprot, int skip,
    double stress[]
) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int k = 0; k < dim; k++) {
            sims[strd_j + k] = sims[strd_j + k] - p * kin.I_n * kin.Cinv[k];
        }
        ...
        strd_j = strd_j + dim;
        (void)muscle.stress(kin, &sims[strd_j]);
    }
    ...
    for (int m = 0; m < nprot; m++) {
        ...
        for (int i = index[select[m]]; i < index[select[m] + 1]; i += skip) {
            ...
            for (int j = 0; j < nlaws; j++) {
                strd_j = j * dim;
                for (int k = 0; k < dim; k++) {
                    stress[strd_i + k * nlaws + j] =
                        sqrt(weights[strd_m + k]) * sims[strd_i + strd_j + k];
                }
            }
        }
    }
