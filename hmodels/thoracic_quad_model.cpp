...
#include <cmath>
#include "thoracic_quad_model.hpp"
...
/*----------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------*/
    void ThoracicQuadBase::get_scaled_pars(double pars[])
    {
        pars[0] = m_elastin.mu;
        pars[1] = m_muscle.k;
    ...
    void ThoracicQuadBase::stress(const kinematics::kinematics<4> &kin, const double dt, double stress[])
    {
        double p;
    ...
    void ThoracicQuadVEBase::stress(const kinematics::kinematics<4> &kin, const double dt, double stress[])
    {
        double p;
        double el[4], smc[4], col[4];
        p = m_elastin.stress(kin, el);
