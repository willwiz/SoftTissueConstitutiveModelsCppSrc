...
namespace residuals
{
  ...
  template <class matlaw, ResidualCalculation resfunc, HysteresisCalculation hystfunc,
      ...
      int index[], int select[], int n, int dim, int nprot, int skip)
  {
    ...
    double hyst = hystfunc(sims, deltaCG, hysteresis, weights, index,
  ...
  double residual_general(
      double strain[], double stress[], double weights[],
  ...
  // Residual Calculation
  double residual_body_below(double strain[], double stress[], double weights[],
                             int index[], int select[], int dim, int nprot, int skip,
                             double sims[])
  {
  ...
  double residual_body_below2(double strain[], double stress[], double weights[],
                              int index[], int select[], int dim, int nprot, int skip,
                              double sims[])
  {
  ...
  double hysteresis_body_null(double sims[], double deltaCG[], double hysteresis[], double weights[],
                              int index[], int select[], int dim, int nprot, int skip)
  {
    return 0.0;
  }
  ...
  double hysteresis_body(double sims[], double deltaCG[], double hysteresis[], double weights[],
                         int index[], int select[], int dim, int nprot, int skip)
  {
  ...
  double penalty_body_kappa_2(double pars[], double fiber[], double visco[])
  {
    ...
    return 1.0 + 1.0 * d_fiber * d_fiber + 0.1 * d_elastin * d_elastin + 40.0 * d_kappa * d_kappa + 10.0 * (d_kappa2 * d_kappa2);
  }
  ...
  //   return 1.0 + 0.01 * fiber[0] * fiber[0] + 0.1 *d_kappa * d_kappa + 1.0 * (d_kappa2*d_kappa2) + 1.0 * d_alpha * d_alpha;
  // }
  ...
  double penalty_body_kappa_1(double pars[], double fiber[], double visco[])
  {
    ...
    return 1.0 + ctv::p_fiber * fiber[0] * fiber[0] + 0.001 * d_kappa * d_kappa + ctv::p_alpha * d_alpha * d_alpha + ctv::p_collagen * pars[1] * pars[1];
  }
  ...
  double penalty_body_1_3(double pars[], double fiber[], double visco[])
  {
    ...
    return 1.0 + ctv::p_fiber * fiber[0] * fiber[0] + ctv::p_alpha * d_alpha * d_alpha + ctv::p_elastin * d_weight * d_weight;
  }
  ...
  double penalty_body_2_4(double pars[], double fiber[], double visco[])
  {
    ...
    return 1.0 + ctv::p_fiber * fiber[0] * fiber[0] + 0.1 * d_alpha * d_alpha + 2.0 * d_kappa * d_kappa + 0.001 * d_elastin * d_elastin + 0.001 * d_weight * d_weight;
  }
                       ...
                       int index[], int select[], int n, int dim, int nprot, int skip)
  {
    ...
    double res = residual_general<norm_func>(args, stress, weights, index, select, dim, nprot, skip, sims);
  ...
  double calc_residual_hyst(double pars[], double fiber[],
                            double visco[], double Tf, double Cmax[],
                            double args[], double stress[], double dt[], double weights[],
                            double deltaCG[], double hysteresis[], double alphas[],
                            int index[], int select[], int n, int dim, int nprot, int skip)
  ...
  //   return (res + ctv::w_hyst * scale * hyst) * (penalty_body(pars, fiber, visco) + ctv::w_visco * alp);
  // }
